...
.if exists(${OBJ_DIR}/${APIS}/building/${API}.api/Makefile)
. include "${OBJ_DIR}/${APIS}/building/${API}.api/Makefile"
.endif

# Make tspec-generated paths relative to objdir
.if "${APIOBJS}" != ""
APIOBJS:=	${APIOBJS:C/^/${OBJ_DIR}\/${APIS}\//g}
APILIB:= 	${APILIB:C/^/${OBJ_DIR}\/${APIS}\//g}
.endif
...
all::
	...
	@cd ${OBJ_DIR}/${APIS} && ${TSPEC} -v -I${BASE_DIR}/${APIS} -O. -S./building ${API}
	@cd ${BASE_DIR}/${APIS}/${API}/ && ${.MAKE} makeapi ${.MAKEFLAGS}


clean::
	@cd ${BASE_DIR}/${TOKENS_MACH} && ${MAKE} clean
	${REMOVE} ${OBJ_DIR}/${APIS}/building/${API}.api/Makefile ${APILIB} \
		${APIOBJS} ${APIOBJS:S/.j/.c/}


# Relative to .OBJDIR.
CAPIDIR=	${OBJ_DIR}/${APIS}/building
SAPIDIR=	${OBJ_DIR}/${APIS}/shared
HAPIDIR=	${OBJ_DIR}/${APIS}
...
.if exists(${SAPIDIR}/${API}.api)
	@${CONDCREATE} "${SINSTDIR}"
.endif
	${INSTALL} -m 644 ${OBJ_DIR}/${APIS}/${API}.tl ${LINSTDIR}
