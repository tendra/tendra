 ...
 */
static
void code_for_ret(int which_ret)
{
	if (which_ret == UNTIDY) {
		rr_ins(i_copy, SP, T2);
	}
...
make_proc_tag_code(exp e, space sp, where dest, int exitlab)
{
	...
	if (is_main) {
		...
		baseoff b;
		b.base = 0;
		b.offset = (assembler == ASM_GAS ? 0 : 4);
	...
	if (optim & OPTIM_PEEPHOLE) {
		...
		for (i = 0; i < line; i++) {
			...
			} else if (lab > NA && lab != res_label && pCode[i] ->ins != i_lab) {
...
#endif
			} else if (pCode[i] ->ins == i_cj || pCode[i] ->ins == i_cij) {
				jump = i - to;
				if (SIMM11(jump * 4)) {
					ins_p cc;
					int a, b;
...
make_res_tag_code(exp e, space sp, where dest, int exitlab)
{
	...
	if (e->tag == res_tag) {
		nm = RES;
		ret_label = &res_label;
	} else if (e->tag == return_to_label_tag) {
		nm = TO_LAB;
