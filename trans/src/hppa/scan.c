...
complex(exp e)
{
	...
	if (name(e) == cont_tag && name(son(e)) == name_tag && isvar(son(son(e)))) {
		return 0;
	}

	if (name(e) == val_tag || name(e) == real_tag) {
		return 0;
	}
...
scan_cond(exp * e, exp outer_id)
{
	    ...
	    && name(son(son(son(second)))) == test_tag)
	{
		...
		if (c1 && eq_exp(op11, op12)) {
			...
			exp newid = getexp(sh(ste), bro(ste), last(ste), op11, NULL, 0, 2, ident_tag);
...
fpop(exp *e, exp **at)
{
	...
	if ((has & HAS_LONG_DOUBLE) && name(sh(son(op))) == doublehd) {
		...
		if (!is_o(name(*arg)) || pcl) {
			cca(at, arg);
		}

		arg = &bro(son(op));
		if (!is_o(name(*arg)) || pcr) {
			cca(at, arg);
		}
...
scan(exp *e, exp **at)
{
		    ...
		    && name(son(son(son(second)))) == test_tag)
		{
			...
			* L:cond(seq(test;...),...) ) ..... */
				...
				/* ....if first operands of tests are same, identify them */
			{
				...
				exp newid = getexp(sh(ste), bro(ste), last(ste), son(test1), NULL,
				                   0, 2, ident_tag);
	...
	case apply_tag: {
		...
		int i;
		bool notinreg = !((has & HAS_LONG_DOUBLE) ? (name(sh(application)) == shrealhd ||
	...
	case val_tag: {
		...
		asm_comment("val_tag %d no=%d", name(s), no(*e));
		/* ... and continue */
	}
...
mult_tag_case:
	...
	case offset_div_tag: {
		...
		if (name(op2) == val_tag  && name(s) == offsethd && al2(s) >= 8) {
			int n = no(op2) / 8;
			if (n == 1) {
				/* offset is one  byte */
				bro(op1) = bro(*e);
				...
				*e = op1;
				...
				return scan(e, at);
			} else if (name(*e) == offset_mult_tag && n > 1 && (n & (n - 1)) == 0)
				if (name(op1) == and_tag
