...
start:
  ...
  case inreg:
	...
	/* source in fix reg, dest is bit address */
	{
	  ...
	  if (!is.adval)
	  {
	    error(ERR_SERIOUS, "no move to var bits");
	  }
	  ...
	  if (bpos + bsize > 32)
	    error(ERR_SERIOUS, "store bits over w-boundary");
  ...
  case notinreg:
	...
	/* source and dest instore */
	{
	  ...
	  if ((al % 8) != 0 || (bits % 8) != 0)
	  {
	    error(ERR_SERIOUS, "move: bits mem to mem move");
	    return NOREG;
	  }
