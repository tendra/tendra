 ...
 */
void
regn(int regs, int rdisp, exp ldname, int le)
{
	int z;
	char **rn;
	UNUSED(rdisp);
	...
	switch (le) {
	case  8: rn = reg_name_byte; break;
	case 16: rn = reg_name_word; break;
	default: rn = reg_name_long; break;
	}
...
void
index_opnd(where whmain, where wh, int sc)
{
...
extn(exp id, int off, int b)
{
	...
	if (PIC_code) {
		asm_printf("%s@%s", et->name, et->extnamed ? "GOT" : "GOTOFF");
		if (off != 0) {
			asm_printf("+%d", off / 8);
		}
		asm_printf("(%s", "%ebx");
...
void
ins1ind(char *i, int le1, where a1)
{
	asm_printf("\t%s *", i);
	operand(le1, a1, 1, 0);
	asm_printf("\n");
...
void
ins1lit(char *i, int le1, where a1)
{
	asm_printf("\t%s ", i);
	operand(le1, a1, 1, 1);
	asm_printf("\n");
...
/* Two operands */
void
ins2(char *i, int le1, int le2, where a1, where a2)
{
	asm_printf("\t%s ", i);
	operand(le1, a1, 1, 0);
	asm_printf(",");
...
/* Three operands */
void
ins3(char *i, int le1, int le2, int le3, where a1, where a2, where a3)
{
	asm_printf("\t%s ", i);
	operand(le1, a1, 1, 0);
	asm_printf(",");
...
void
simplest_set_lab(int labno)
{
	asm_label("%s%d", local_prefix, labno);
}

void
simple_set_label(int labno)
{
	cond1_set = false;
	cond2_set = false;
...
out_branch(int sg, int test_no, int shnm)
{
	...
	if (sg) {
		...
		switch (test_no) {
		case 1: return sg < 0 ? xse  : jle;
		case 2: return sg < 0 ? js   : jl;
		case 3: return sg < 0 ? jns  : jge;
		case 4: return sg < 0 ? xnse : jg;
		case 5: return jne;
		case 6: return je;
...
void
simple_branch(char *j, int labno)
{
 ...
 */
void
branch(int test_no, exp jr, int sg, int shnm)
{
	int fs_dest = (int)fstack_pos_of(jr);
	int good_fs = fstack_pos;
	...
	if (fstack_pos > fs_dest || childno(jr) != stack_dec || fpucon != normal_fpucon || cmp_64hilab >= 0) {
...

	{
		...
		if (*cj == 'j') {
			...
			/* compare with zero, ignoring overflow */
			int nl1 = next_lab();
			simple_branch(js, nl1);
...
void
setcc(int test_no, int sg, int shnm)
{
	char *b;
	if (cmp_64hilab >= 0) {
		int chl = cmp_64hilab;
 ...
 */
void
jmp_overflow(exp jr, int sg, int inv)
{
	int fs_dest = (int)fstack_pos_of(jr);
	int good_fs = fstack_pos;
	int good_fpucon = fpucon;
 ...
 */
void
test_trap(int test_no, int sg, int shnm)
{
	int nl = next_lab();
	int inv_test_no = (flpt_always_comparable ||
			   (shnm < shrealhd || shnm > doublehd))
 ...
 */
void
mult_op(int inc, where rmain, where rind, int sc, where dest)
{
	asm_printf("\tleal ");
	if (inc != 0) {
		asm_printf("%d", inc);
	...
	asm_printf("(");
	if (rmain.where_exp->tag != val_tag || (no(rmain.where_exp) + rmain.where_off) != 0) {
...
void
caseins(int sz, exp arg, int min, int max, int *v, int exhaustive, int in_eax, exp case_exp)
{
	int tab;
	int absent;
	where a;
...
void
const_intnl(int addr, int lab, int off)
{
	if (PIC_code) {
...
void
rotshift64(int shft, int sig, where wshift)
{
	if (wshift.where_exp->tag == val_tag) {
		/* no of places is constant */
...
#endif
			ins3(dsh, 8, 32, 32, mw(zeroe, places), reg1, mw(ind_sp.where_exp, -32));
			ins3(dsh, 8, 32, 32, mw(zeroe, places), reg0, mw(ind_sp.where_exp, -64));
			ins0(popeax);
#ifdef DWARF2
			if (diag == DIAG_DWARF2 && no_frame) {
				dw2_track_pop();
