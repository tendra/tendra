...
evalexp(exp e)
{
	...
	case concatnof_tag : {
		...
		unsigned long a2 = (unsigned long) evalexp(next(son(e)));

		s1 = ashof(sh(son(e)));
		s2 = ashof(sh(next(son(e))));
...
evalconcbitaux(exp e, concbittype b4, bool ro)
{
	...
	case concatnof_tag: {
		...
		rhs = evalconcbitaux(next(son(e)), lhs, ro);
		return rhs;
	}
