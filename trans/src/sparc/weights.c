 ...
 * ident is set up to represent the break point for allocation.
 * A similar process occurs for procedure parameters which have the
 * break value in the forweights field of the parapair of the
 * corresponding procrec.
 *
 ...
 * for which it is worthwhile to allocate this tag into a reg ("regged").
 *
...
tailrecurse:
	...
	case ident_tag: {
		...
		if (e->props & defer_bit) {
			...
			/* usage of tag stored in number of child of
			   load_name (decl) */
			s = child(t);
