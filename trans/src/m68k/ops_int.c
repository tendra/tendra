 ...
 */
static void
add_aux(shape sha, long sz, where inc, where dest)
{
	int instr;
...
/*
 * The constant c is added to the value inc, and the result is stored in dest.
 */
static void
addsub_const_3_args(shape sha, long sz, long c, where inc, where dest, bool use_sub)
{
	if (c == 0) {
		move(sha, inc, dest);
		return;
...
/*
 * The values a1 and a2 of shape sha are added and the result stored in dest.
 */
void
add(shape sha, where a1, where a2, where dest)
{
	...
	if (wh1 == Value) {
		...
		if (is_offset(a1.wh_exp)) {
			v1 /= 8;
		}

		if (wh2 == Value && !have_overflow()) {
			long v2 = nw(a2);
			if (is_offset(a2.wh_exp)) {
				v2 /= 8;
			}
 ...
 */
static void
sub_aux(shape sha, long sz, where a, where dest)
{
	long wha = whereis(a);
	long whd = whereis(dest);
 ...
 */
static void
mult_aux(shape sha, where a, where dest)
{
 ...
 */
static void
mult_clever(where a, where dest, long sf, bool d)
{
	int r;
	where ar;
	mach_op *op1, *op2;
...
/*
 * The register r is multiplied by 2 to the power of p.
 * The flag D1_used is passed on to shift_aux if necessary.
 */
static void
mult_power2(long p, where r, bool D1_used)
{
	switch (p) {
	case 0:
		return;
 ...
 */
static void
mult_utility(long p, long q, where r1, where r2, bool D1_used, bool first_time)
{
	if (first_time) {
		switch (p) {
		case 0:
 ...
 */
static void
mult_const(shape sha, where a1, where a2, where dest)
{
 ...
 */
static void
div_power2(shape sha, long v, where top, where dest)
{
	long n = ilog2(v);
	bool sw;
	where w;
 ...
 */
static void
rem_power2(shape sha, long v, where top, where dest)
{
	bool sw;
	where w;
	long lab, end;
 ...
 */
static bool
rem_power2_1(shape sha, long v, where top, where dest)
{
	bool sw;
	where d0, d1;
	long loop, end, tst;
	...
	if (s && (eq_where(top, D0) || eq_where(top, D1))) {
		return 0;
	}
 ...
 */
static void
euclid(shape sha, where bottom, where top, where quot, where rem, int type, int form)
{
	long v;
	bool b_const = 0;
	bool save_d1 = 0;
	...
	if (sz == 32) {
		...
		if (type != REM) {
			change_var_sh(sha, lsha, qreg, quot);
		}
		if (type != DIV) {
			change_var_sh(sha, lsha, rreg, rem);
		}
	}
 ...
 */
void
rem1(shape sha, where bottom, where top, where dest)
{
 ...
 */
static void
maxmin(shape sha, where a1, where a2, where dest, int tst)
{
	where d;
	bool sw;
 ...
 */
void
absop(shape sha, where a, where dest)
{
	where d;
	bool sw;
	long lab;
