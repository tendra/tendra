...
evalexp(exp e)
{
	...
	case chvar_tag:
		return correct_shape(evalexp(child(e)), sh(e)->tag);
	...
	case not_tag: return correct_shape(~evalexp(child(e)), sh(e)->tag);
	...
	case shr_tag: {
		...
		unsigned long ul;
		if (sgned) {
			sl = (long)correct_shape(evalexp(child(e)), sh(e)->tag);
			return sl >> evalexp(next(child(e)));
		} else {
			ul = (unsigned long)correct_shape(evalexp(child(e)), sh(e)->tag);
			return ul >> evalexp(next(child(e)));
		}
	}

	case shl_tag:
		return correct_shape(evalexp(child(e)) << evalexp(next(child(e))), sh(e)->tag);
...
static concbittype
addconcbitaux(unsigned long w, int size, concbittype before)
{
	...
	if ((size == 0 && (wordbitposn != 0 || before.value_size != 0))
	   || (wordbitposn + size > 32))
	{
		/*
...
set_align(int al)
{
	...
	case 1:
	case 8:                           break;
	case 16: asm_printop(".align 1"); break;
	case 32: asm_printop(".align 2"); break;
	case 64: asm_printop(".align 3"); break;
...
evalone(exp e, int bitposn)
{
	...
	case string_tag: {
		...
		if (char_size != 8) {
			...
			for (i = 0; i < strsize; i++) {
				...
				switch (char_size) {
				case 16: c = ((unsigned short *) st)[i]; directive = ".short"; break;
				case 32: c = ((unsigned int   *) st)[i]; directive = ".long";  break;
	...
	case name_tag: {
		...
		/* no() is offset */
		if (no(e) == 0) {
			asm_printop(".long %s", name);
		} else {
			asm_printop(".long %s+%ld", name, (long)(no(e) / 8));
		}
