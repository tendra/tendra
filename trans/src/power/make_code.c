...
tailrecurse:
	...
	case seq_tag: {
		...
		for (t = son(son(e)); ; t = next(t)) {
			...
			if (t->last) {
				exp l = next(son(e)); /* last exp of sequence */
	...
	case rep_tag: {
		...
		if (cpu != CPU_POWERPC) {
			...
			last_test = next(son(second));	/* under labst_tag */
			/* dive down sequences */
			while (last_test->tag == seq_tag) {
				last_test = next(son(last_test));
			}
	...
	case goto_tag: {
		...
		while (next(son(gotodest))->tag == goto_tag) {
			/* goto to goto optimisation */
			gotodest = pt(next(son(gotodest)));
		}
#endif
	...
	case shr_tag: {
		...
		if (s->tag == and_tag && b->tag == val_tag &&
		    next(son(s))->tag == val_tag &&
		    is_a_mask(no(next(son(s)))) &&
		    shape_size(sh(e)) == 32) {
			unsigned int mask = (unsigned int)no(next(son(s)));
	...
	case and_tag: {
		    ...
		    next(son(arg1))->tag == val_tag)
		{
			...
			if (arg1->tag == shl_tag) {
				int shift_left = no(next(son(arg1)));
				if (shift_left <= mask_right) {
					rotation_left = shift_left;
					use_rlinm_ins = 1;
				}
			} else if (arg1->tag == shr_tag) {
				int shift_right = no(next(son(arg1)));
	...
	case local_free_tag: {
		...
		if (next(son(e))->tag == val_tag) {
			int displacement = ALLOCA_ALIGNMENT(no(next(son(e))));
			...
			nsp = guardreg(r, sp);
			off = reg_operand(next(son(e)), nsp);
