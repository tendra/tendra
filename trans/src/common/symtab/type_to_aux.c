...
type_size(diag_type dt)
{
	...
	case DIAG_TYPE_ARRAY: {
		return type_size(dt->data.array.element_type)
			* (dt->data.array.upper_b-dt->data.array.lower_b + 1);
	}
...
add_type_to_aux(diag_type dt, long ind)
{
	...
	while (type->bt == 63) {
		...
		case DIAG_TYPE_BITFIELD:{
				...
				retaux.rndx.rfd =
					dt->data.bitfield.no_of_bits.nat_val.small_nat;
				retaux.rndx.index = 0;
				add_aux(retaux, ind);
				break;
			}
