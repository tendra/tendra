...
{
	exp n = getexp(f_top, bro(e), (int) (last(e)), NULL, NULL, 0, 0, top_tag);
			...
			if (last(c) && c->tag == cond_tag) {
				...
				if (no(son(second)) == 1 &&
					...
					if (r->tag == ass_tag &&
					    ...
					    !isbigval(bro(son(r))) &&
					    no(bro(son(r))) == 1 &&
					    last(s) && s->tag == test_tag &&
					    pt(s) == second) {
						return s;
					}
				} /* cond is (seq= first | L: top) */
			} /* seq is cond=c; id */
			if (last(c) && c->tag == condassign_tag) {
	...
	case test_tag: {
		...
		if (endian == ENDIAN_LITTLE && has & HAS_BYTEOPS) {
			...
			if (arg2->tag == val_tag && !isbigval(arg2) && no(arg2) == 0 &&
				...
				for (t = son(arg1); !last(bro(t)); t = bro(t))
					;
