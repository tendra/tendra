...
init_frame_als(void)
{
	...
	for (i = 0; i < 32; i++) {
		...
		frame_als[i].al.state = ALDEF_VAL;
		frame_als[i].al.u.val = 64;
...
f_alignment(shape sha)
{
  ...
  */
alignment
f_obtain_al_tag(al_tag a)
{
	alignment j;
	if (a->al.state == ALDEF_VAL) {
		return long_to_al(a->al.u.val);
	}
...
alignment
f_unite_alignments(alignment a, alignment b)
{
	alignment j;
	if (a->al.state == ALDEF_VAL && b->al.state == ALDEF_VAL) {
		if (a->al.frame == b->al.frame) {
			if (a->al.u.val > b->al.u.val) {
				return a;
			} else {
				return b;
			}
		} else if (a->al.frame == 0) {
			return b;
		} else if (b->al.frame == 0) {
			return a;
		} else {
			return &frame_als[(a->al.frame | b->al.frame) - 1];
		}
	}
...
access
f_add_accesses(access a, access b)
{
	return a | b;
}
...
init_alignment(void)
{
	const_al1->al.state = ALDEF_VAL;
	const_al1->al.u.val = 1;
	const_al1->al.frame = 0;
	const_al1->al.sh_hd = 0;

	const_al8->al.state = ALDEF_VAL;
	const_al8->al.u.val = 8;
	const_al8->al.frame = 0;
	const_al8->al.sh_hd = 0;

	const_al16->al.state = ALDEF_VAL;
	const_al16->al.u.val = 16;
	const_al16->al.frame = 0;
	const_al16->al.sh_hd = 0;

	const_al32->al.state = ALDEF_VAL;
	const_al32->al.u.val = 32;
	const_al32->al.frame = 0;
	const_al32->al.sh_hd = 0;

	const_al64->al.state = ALDEF_VAL;
	const_al64->al.u.val = 64;
	const_al64->al.frame = 0;
	const_al64->al.sh_hd = 0;

	const_al512->al.state = ALDEF_VAL;
	const_al512->al.u.val = 512;
...
f_apply_proc(shape result_shape, exp arg1, exp_list arg2, exp_option varparam)
{
	...
	if (arg1->tag == name_tag && isglob(child(arg1)) && !isvar(child(arg1))) {
		exp e;
	...
	if (arg2.number==0) {
		...
		if (promote_pars) {
			promote_actuals(next(child(res)));
		}
	}
	...
	if (redo_structfns && !reg_result(result_shape)) {
		...
		appname = getexp(ptr_res_shape, next(child(res)), 0, vardec,
				 contname, 0, 0, name_tag);
		++no(vardec);
		pt(vardec) = appname;
		app = getexp(f_top, NULL, 0, child(res), NULL, 0, 32,
			     apply_tag);
		if (child(res)->last) {
			child(res)->last = false;
			appname->last  = true;
			next(appname) = app;
		}
		next(child(res)) = appname;
		t = child(app);
		list.number = 1;
		while (!t->last) {
			t = next(t);
...
f_assign_with_mode(transfer_mode md, exp arg1, exp arg2)
{
	...
	if ((md & f_volatile) != 0) {
		return me_b3(f_top, arg1, arg2, assvol_tag);
		   ...
		   !reg_result(sh(arg2))) {
		return f_move_some(md, child(arg2), arg1,
...
f_bitfield_assign(exp p, exp off, exp val)
{
	...
	if (off->tag == val_tag) {
		...
		exp idoff = getexp(f_top, NULL, 0, off, noff2, 0, 2,
				   ident_tag);
		child(noff1) = idoff; child(noff2) = idoff;
		{
			exp v, idbnt;
			exp pn1, pn2, idpn;
			...
			child(nby1) = idby; child(nby2) = idby; child(nby3) = idby;
			child(bnt1) = idbnt; child(bnt2) = idbnt;
			child(pn1) = idpn; child(pn2) = idpn;
			next(child(idby)) = idbnt; child(idby )->last = false;
			next(child(idbnt)) = idpn; child(idbnt)->last = false;
			next(child(idoff)) = idby; child(idoff)->last = false;
			...
			next(child(idpn)) = asit; child(idpn)->last = false;
	...
	if (off->tag == val_tag) {
		...
		child(noff1) = idoff; child(noff2) = idoff;
		{
			...
			exp v1bit = getexp(ob, NULL, 0, NULL, NULL, 0, 1,
					   val_tag);
			child(nby1) = idby; child(nby2) = idby;
...
f_case_transform(bool exhaustive, exp control, caselim_list branches)
{
	...
	while (branches != NULL) {
		...
		sh(hd) = sh(control);
		if (child(hd) != NULL) {
			sh(child(hd)) = sh(control);
		}
		if (child(hd) != NULL &&
		    docmp_f((int)f_less_than, child(hd), hd)) {
			--no(child(pt(hd)));
			retcell(child(hd));
			retcell(hd);
		} else {
	...
	while (next(ht) != NULL) {
		...
		sh(ht) = changer_shape;
		if (child(ht) != NULL) {
			sh(child(ht)) = changer_shape;
		}
	}
	ht->last = true;
	next(ht) = r;

	control_expression = child(r);
	body_of_case = next(child(r));
...
f_env_offset(alignment fa, alignment y, tag t)
{
	...
	if (e == NULL) {
		e = getexp(f_bottom, NULL, 0, NULL, NULL, 0, 0,
			   ident_tag);
		child(e) = e; /* used to indicate that tag is not yet defined!?*/
		set_tag(t, e);
	}
...
f_labelled(label_list placelabs_intro, exp starter, exp_list places)
{
	...
	for (i = 0; i < places.number; ++i) {
		...
		setnext(child(labst), f);
		setnext(f, labst);
		f->last = true;
		setsh(labst, sh(f));
		if (starter->tag == case_tag ||
		    (starter->tag == seq_tag &&
		     child(child(starter))->tag == case_tag)) {
	    ...
	    tagacc_option vartag, exp body)
{
	...
	if (params_intro.number != 0) {
		...
		for (param = params_intro.last_id; param != res;
...
#if TRANS_HPPA
			    (varhack || ((shape_size(sh(child(param))) >64) &&
					 (sh(child(param))->tag == cpdhd ||
					  sh(child(param))->tag == nofhd ||
					  sh(child(param))->tag == doublehd))))
#else
#if TRANS_SPARC
			    (varhack || sparccpd(sh(child(param)))))

#else
			    (varhack || sh(child(param))->tag == cpdhd ||
			     sh(child(param))->tag == nofhd ||
			     sh(child(param))->tag == doublehd))
#endif
#endif
			{
				...
				exp eo = NULL;
				shape ptr_s =
				    f_pointer(f_alignment(sh(child(param))));
	...
	if (proc_struct_result != NULL) {
		next(child(proc_struct_result)) = child(res);
		setfather(proc_struct_result, child(res));
		child(res) = proc_struct_result;
...
#if TRANS_HPPA
			    shape_size(sh(child(param))) > 64)
#else
				(sh(child(param))->tag == cpdhd ||
				 sh(child(param))->tag == nofhd ||
#if TRANS_SPARC
				 sparccpd(sh(child(param))) ||
#endif
				 sh(child(param))->tag == doublehd))
#endif
			{
	...
	if (redo_structfns && !reg_result(result_shape)) {
		if (proc_struct_result==NULL) {
			exp init = getexp(f_pointer(f_alignment(result_shape)),
					  ...
					  clear_tag);
			exp iddec = getexp(sh(child(res)), NULL, 0, init,
					   NULL, 0, 0, ident_tag);
			setparam(iddec);
...
find_caller_id(int n, exp p)
{
	while (p->tag == ident_tag) {
		if (child(p)->tag == caller_name_tag && no(child(p)) ==n) {
			return p;
		}
		p = next(child(p));
	}
		     ...
		     exp postlude)
{
	...
	if (redo_structparams) {
		...
		for (i = 0; i < caller_pars.number; i++) {
			exp ote = *plce;
			exp param = (ote->tag == caller_tag) ? child(ote) : ote;
	...
	if (caller_pars.id != NULL) {
		...
		while (next(child(a)) != NULL) {
			a = next(child(a));
		}
		next(child(a)) = postlude;
	...
	if (redo_structfns && !reg_result(result_shape))
	{
		...
		appname = getexp(ptr_res_shape, child(r_p), 0, vardec, contname,
				 0, 0, name_tag);
		if (no(r_p)++ == 0) {
	...
	if (redos != NULL) {
		...
		next(child(last_redo)) = res;
		child(last_redo)->last = false;
		next(res) = last_redo;
		res->last = true;
		res = redos;
	}
...
{
	alignment a = al1_of(sh(arg1));
	alignment b = al1_of(sh(arg2));
	alignment c = al2_of(sh(arg1));
	shape sha;
	if (sh(arg1)->tag == bothd) {
		kill_exp(arg2, arg2);
		return arg1;
	...
	if (a->al.state != ALDEF_VAL || b->al.state != ALDEF_VAL) {
		...
		sha = f_offset(long_to_al(MAX(a->al.u.val, b->al.u.val)), c);
	}
...
f_offset_pad(alignment a, exp arg1)
{
	...
	if (a->al.state != ALDEF_VAL || al1_of(sh(arg1))->al.state != ALDEF_VAL) {
		alignment ares = (alignment)calloc(1, sizeof(aldef));
		if (!doing_aldefs) {
		...
		ares->al.state = ALDEF_JOINAB;
		ares->al.u.join.a = a;
		ares->al.u.join.b = al1_of(sh(arg1));
		ares->next = top_aldef;
		top_aldef = ares;
		sha = f_offset(ares, a);
		...
		sha = f_offset(long_to_al(MAX(a->al.u.val, al1(sh(arg1)))), a);
	}
...
f_repeat(label repeat_label_intro, exp start, exp body)
{
	...
	next(start) = labst;
	start->last = false;
	setnext(child(labst), body);
	child(labst)->last = false;
	setnext(body, labst);
	body->last = true;
	setsh(labst, sh(body));
	child(crt_repeat) = r;
...
f_computed_nat(exp arg)
{
	...
	if (arg->tag == name_tag && !isvar(child(arg))) {
		res = f_computed_nat(child(child(arg)));
		kill_exp(arg, arg);
		return res;
	}
...
f_offset(alignment arg1, alignment arg2)
{
	...
	if (arg1->al.state != ALDEF_VAL || arg2->al.state != ALDEF_VAL || arg1->al.sh_hd != 0 ||
	...
	/* use values pre-computed by init since we never alter shapes */
	switch (arg1->al.u.val) {
	case 512:
		switch (arg2->al.u.val) {
...
f_pointer(alignment arg)
{
	/* use values pre-computed by init since we never alter shapes */
	int af = arg->al.frame;
	if (arg->al.state != ALDEF_VAL && af == 0) {
...
f_computed_signed_nat(exp arg)
{
	...
	if (arg->tag == name_tag && !isvar(child(arg))) {
		res = f_computed_signed_nat(child(child(arg)));
		kill_exp(arg, arg);
		return res;
	}
...
string
f_concat_string(string a, string b)
{
	int i;
	string res;
	if (a.size != b.size) {
	...
	if (res.size <= 8) {
		...
		for (i = 0; i < a.number; i++) {
			res.ints.chars[i] = a.ints.chars[i];
		}
		for (i = 0; i < b.number; i++) {
			res.ints.chars[i + a.number] = b.ints.chars[i];
		}
		res.ints.chars[res.number] = 0;
	} else if (res.size <= 16) {
		res.ints.shorts = xcalloc(res.number + 1, sizeof(short));
		for (i = 0; i < a.number; i++) {
			res.ints.shorts[i] = a.ints.shorts[i];
		}
		for (i = 0; i < b.number; i++) {
			res.ints.shorts[i + a.number] = b.ints.shorts[i];
		}
		res.ints.shorts[res.number] =0;
	} else {
		res.ints.longs = xcalloc(res.number + 1, sizeof(int));
		for (i = 0; i < a.number; i++) {
			res.ints.longs[i] = a.ints.longs[i];
		}
		for (i = 0; i < b.number; i++) {
			res.ints.longs[i + a.number] = b.ints.longs[i];
		}
...
add_caselim_list(caselim_list list, caselim elem, int index)
{
	 ...
	 */
	++no(child(pt(lowval)));	/* record label use */
	child(lowval) = ht;
	/*    case_item (lowval);
	 */
	next(lowval) = list;
...
tidy_initial_values(void)
{
	...
	for (d = top_def; d != NULL; d = d->next) {
		...
		if (child(crt_exp) != NULL && d->extnamed) {
			good_name = d->name;
		}
		if (child(crt_exp) != NULL &&
		    child(crt_exp)->tag == initial_value_tag) {
			...
			{
				exp init = child(child(crt_exp));
				exp new_init = f_make_value(sh(init));
				if (good_name == NULL) {
					good_name = d->name;
				}
				retcell(child(crt_exp));
				child(crt_exp) = new_init;
		...
		if (do_prom && child(crt_exp) != NULL && d->var && !is_comm(child(crt_exp))) {
			/* accumulate assignments of non-zero initialisations
			   in one explist */
			exp init = child(crt_exp);
			exp new_init = f_make_value(sh(init));
			if (good_name == NULL) {
				good_name = d->name;
			}
