...
kill_exp(exp e, exp scope)
{
	...
	if (n == case_tag) {
		...
		for (b = next(son(e)); b != NULL; b = nextb) {
			int l;
...

	{
		...
		if (p != NULL && (props(son(p)) & 1) == 0) {
			...
			no(son(p))--;
			if (no(son(p)) == 0 && !is_loaded_lv(p) && next(son(p)) != NULL &&
...
case_item(exp i)
{
	...
	if (next(t) != NULL) {
		nlow = next(t);
		nhigh = (son(next(t)) == NULL) ? nlow : son(next(t));
	} else {
		SET(nlow);
		SET(nhigh);
	}
...
scan_solve(exp e)
{
	...
	case ident_tag:
		scan_solve(son(e));
		scan_solve(next(son(e)));
		return;

	case case_tag: {
		exp t;

		for (t = son(e); !t->last; t = next(t)) {
			exp s = son(pt(next(t)));
			if (isvar(s)) {
				++no(s);
			}
	...
	default:
		...
		if (son(e) != NULL) {
			...
			for (t = son(e); scan_solve(t), !t->last; t = next(t))
				;
		}
...
exp copy_cpd(exp e, exp new_record, exp var, exp lab)
{
	...
	q = next(son(e));
	while (j = copy_res(next(son(q)), var, lab), c = pt(q), next(son(c)) = j,
...
copy_res(exp e, exp var, exp lab)
{
	...
	if (n == ident_tag) {
		...
		bs = copy_res(next(son(e)), var, lab);/* copy the body */
...
#if TDF_DIAG4
				if (extra_diags) {
					diag_inline_result(next(son(ass)));
				}
#endif
