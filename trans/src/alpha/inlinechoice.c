...
#undef QQQ
	...
	for (;;) {
		...
		case cont_tag:
			if (child(apars)->tag == name_tag && isvar(child(child(apars))) && !isvar(fpars)) {
				break;
			}
		...
		case val_tag: {
			...
			/*
			 * Simple constant param. Increase desire to inline since
			 * a constant may cause further optimisation,
