  ...
  switch (name (e)) {
	...
	if (longs == 0) {
	  ...
	  if (name(cees) == same_callees_tag) {
	    ...
	    if (callee_size >= 0 && call_has_vcallees(cees)) {
	      outs ("pop ecx\n");
	      outs ("lea edx, "); outn((long)callee_size/8); outs("(esp)\n");
	      outs ("push edx\n");
	      outs ("push ecx\n");
	    }
	  }
	  ...
	  outs ("push ");
	  rel_ap (0);	/* push return address after callees */
	  outnl();
	  stack_dec -= 32;
	...
	cond2_set = 0;
	{
	  ...
	  if (reg_result (sh (son (e)))) {/* answer to registers */
		...
		regsinuse = old_regsinuse;
		outs ("push ");
		rel_ap (0);	/* push return address for return after pops */
		outnl();
#ifdef NEWDWARF
		if (diagnose && dwarf2)
		  dw2_untidy_return ();
