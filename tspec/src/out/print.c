...
#define SRC_TO_INCLUDE "../../include"
#define SRC_TO_SUBSET "../../subset"
...
print_object(FILE *output, object *input, int pass)
{
	...
	for (p = input; p != NULL; p = p->next) {
		...
		case OBJ_USE: {
			...
			if (i->subset != NULL) {
				n = strlen(output_subset_dir) + 1;
			} else {
				n = strlen(output_incl_dir) + 1;
			}

			if (i->subset != NULL) {
				dir = INCLUDE_TO_SUBSET;
			} else {
				dir = SUBSET_TO_SUBSET;
			}
...
print_set(object *input, int pass)
{
	...
	if (pass == 0) {
		...
		if (i->method == NULL) {
			...
			if (i->subset != NULL) {
				s = i->incl + strlen(output_subset_dir) + 1;
			} else {
				s = i->incl + strlen(output_incl_dir) + 1;
			}
